...
class Paginacao { 
    ...
    var $TextoAnterior = '';                // Texto a ser mostrado no link para a página anterior
    ...
    function MontarPaginacao($pgatual = 0, $pgfim = 0) {
        ...
        */
        ...
        if (($this->PaginaAtual - ($this->NumPgLaterais + 1) > 1) && ($this->UltimaPagina > ($this->NumPgLaterais * 2 + 2))) {
            $primeira = '<div class="NumPag"><A HREF="'.$link.'1" CLASS="'.$this->ClasseNavegadores.'">1</A></div>';
        ...
        if (($this->PaginaAtual + ($this->NumPgLaterais + 1) < $this->UltimaPagina) && ($this->UltimaPagina > ($this->NumPgLaterais * 2 + 2))) {
            $ultima = '<div class="NumPag"><A HREF="'.$link.$this->UltimaPagina.'" CLASS="'.$this->ClasseNavegadores.'">'.$this->UltimaPagina.'</A></div>';
        ...
        for ($x = $this->PaginaAtual - $dec; $x <= $this->PaginaAtual + $inc; $x++) {
            if ($x == $this->PaginaAtual) {
                $atual .= '<li class="page-item disabled numeros"><A HREF="'.$link.$x.'" class="page-link">'.$x.'</A></li> ';  
            } else {
                $atual .= '<li class="page-item numeros"><A HREF="'.$link.$x.'" class="page-link">'.$x.'</A></li>';
            }
        }
