<?php
	...
	if($tag != ''){
		...
		#####consulta total de filmes
		$query_paginacao = "SELECT * FROM cenas INNER JOIN categorias_conteudo ON cenas.id = categorias_conteudo.id_conteudo WHERE categorias_conteudo.id_categoria = 40 AND `exibicao`='Todos' AND status='Ativo' $complemento";
		$consulta_paginacao = mysql_query($query_paginacao);
		$totalVideos_cena = mysql_num_rows($consulta_paginacao);
		$totalPaginas = ceil($totalVideos_cena / 24);
		...
		$query_cenas = "SELECT * FROM cenas INNER JOIN categorias_conteudo ON cenas.id = categorias_conteudo.id_conteudo WHERE categorias_conteudo.id_categoria = 40 AND `exibicao`='Todos' AND status='Ativo' $complemento LIMIT $inicio_consulta, 24";
		...
		<div class="container">
			...
			<div class="row" style="float:left;width:100%">
				<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
					...
					<span class="icone-pimenta-titulo"></span>CENAS AMADORAS</h1>
				</div>	
			</div>	
				...
				<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 container tres-botoes container-table-cenas">						
					<div class="col-lg-9 col-md-9 col-sm-12 col-xs-12 coluna-tres-botoes">							
						<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4 modelos">								
							<a href="<?php echo URL.'amadores/?'?>s=v"><button type="button" class="tres-botoes-modelos float-right"><?php echo M_VISTOS ?></button></a>
						</div>						
						<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4 modelos ordem-alfabetica">								
							<a href="<?php echo URL ?>amadores/?s=a"><button type="button" class="tres-botoes-modelos"><?php echo O_ALEATORIA ?></button></a>							
							</div>							<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4 modelos">							
							<a href="<?php echo URL ?>amadores/?s=r"><button type="button" class="tres-botoes-modelos"><?php echo M_RECENTES ?></button></a>
						</div>						
					</div>					
				</div>
			</div>
					...
					if($tag != ''){
						while($dados_categoria_conteudo = mysql_fetch_array($consulta_categoria_conteudo)){
							...
							<div class="col-lg-4 col-md-4 col-sm-6 col-xs-6 cenas inicial_box_conteudo">
								...
								<div>									
									<a href="<?php echo utf8_encode(URL.'cena/'.$dados_cenas_tag[id].'/'.URL_amigavel($dados_cenas_tag[titulo])) ?>" class="<?php if($dados_cenas_tag[video_preview] != '') echo ' item-video '; if($dados_cenas_tag[video_preview_gif] != '') echo ' item-video-gif ' ?>">										
									...
									<div class="paginas-titulo-visualizacoes">										
										<h4 class="paginas-titulo">											
											<a href="<?php echo utf8_encode(URL.'cena/'.$dados_cenas_tag[id].'/'.URL_amigavel($dados_cenas_tag[titulo])) ?>">
							...
							<?php 
								...
								<div class="col-lg-4 col-md-4 col-sm-6 col-xs-6 cenas inicial_box_conteudo">		
									...
									<div>									
										...
										<div class="paginas-titulo-visualizacoes">										
											<h4 class="paginas-titulo">											
												<a href="<?php echo URL.'cena/'.$row_conteudo[id].'/'.URL_amigavel($row_conteudo[titulo]) ?>">
								...
								<div class="col-lg-4 col-md-4 col-sm-6 col-xs-6 cenas inicial_box_conteudo">		
									...
									<div>									
										<a href="<?php echo utf8_encode(URL.'cena/'.$row_conteudo[id].'/'.URL_amigavel($row_conteudo[titulo])) ?>" class="<?php if($row_conteudo[video_preview] != '') echo ' item-video '; if($row_conteudo[video_preview_gif] != '') echo ' item-video-gif ' ?>">										
