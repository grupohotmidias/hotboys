<?php
	...
	$sql_usuario = "SELECT * FROM gozei WHERE id_user='$id_cliente' AND id_cena='$id'";
	$consulta_usuario = mysql_query($sql_usuario);
	$resultado_usuario = mysql_fetch_array($consulta_usuario);
	
	if(isset($resultado_usuario['id_user'])){
		if(isset($resultado_usuario['id_cena'])){
			$msg2 = "Você já gozou com este vídeo hoje";
		}
	}
	elseif(isset($gozada) && $gozada != ""){
		mysql_query("INSERT INTO `gozei`(`id_votacao`, `id_cena`, `id_user`, `cliques`, `data` ) VALUES ('null','$id','$id_cliente','$gozada','$data')");
		if($gozada == 1){$msg = "Que Tesão!!";}
	}
?>
...
<html lang="pt-BR">
    ...
    text-align: center;
    font-size: 1.4rem;
    margin: 0px;" ><span>Gozou com a cena?</span>💦GOZEI</span>
    ...
    text-align: center;
    font-size: 1.4rem;
    margin: 0px;" ><span>Colocar como favorita?</span>⭐ FAVORITO</span>
											
											<span class="col-4">botao3</span> 
										</div>
									</div>
									
								</div>
								<?php if(isset($resultado_usuario['id_user']) ==""){ ?>
								<form action="" method="POST" style="position:relative;float:left;">
									<input type="hidden" name="gozada" value="1">
									<button type="submit" class="sim" style="text-decoration:none;position: relative;border:0px;border-radius: 10px;left: 11px;">Gozei</button>
								</form>
								<?php echo $msg ?>
								<?php }else { ?>
								<?php echo $msg2 ?>
											...
											<div class="page-title-wrapper">
												...
												<?php if($total_modelo_associador_cenas >= 1){ ?>
													<div class="page-title-heading">
														...
														<h5>Mais cenas com <a href="<?php echo utf8_encode(URL_VIP.'modelo/'.$dados_modelo_cenas[id].'/'.URL_amigavel($dados_modelo_cenas[nome])) ?>"><?php echo utf8_encode($dados_modelo_cenas['nome'])?>
														</h5>
														</div>
													<?php } ?>
