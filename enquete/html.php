...
class Html extends Bancodedados{
	...
	if(is_resource($result) && mysql_num_rows($result) > 0){
	    ...
	    while ($row = mysql_fetch_array($result)) {
            echo '<label for="option-'.$row['id'].'" ><div style="height: 57px;width: 16%;border-radius: 198px;overflow: hidden;"><img style="width:100%;float: left;" src="https://server2.hotboys.com.br/arquivos/'.$row['img'].'"></div></label>';
		echo '<input type="radio" name="poll" value="'.$row['id'].'" id="option-'.$row['id'].'" style="position: relative;left: 60px;top: -40px;" />
		<label for="option-'.$row['id'].'" style="position: relative;left: 60px;top: -40px;color: white;font-size: 25px;font-family: sans-serif;font-weight: bold;" >'.utf8_encode($row['nome']).'</label>';
	    }
        echo '<input type="submit" <input type="submit" value="Votar" style="background-color: #dc0505;border: transparent;border-radius: 5px;width: 95%;height: 24px;color: white;font-family: sans-serif;font-size: initial;font-weight: bold;" value="Votar" /></form>';
